...
func TestServerDebugLogsCommands(t *testing.T) {
	...
	for time.Now().Before(deadline) {
		if strings.Contains(buf.String(), "server[n1] command op=set key=a") {
			return
		}
		time.Sleep(10 * time.Millisecond)
	}
	t.Fatalf("expected debug command log, got: %s", buf.String())
}
